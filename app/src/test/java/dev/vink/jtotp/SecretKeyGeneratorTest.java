...
public class SecretKeyGeneratorTest {
    ...
    @Test void KeyGenerate32bytes() {
        String secretString = SecretKeyGenerator.generate();
        assertEquals(secretString.length() , 32 , "Secret key size not as expected. Expected : 32 , actual : " + secretString.length() );
    }

    @Test void KeyGenerateRandom() {
        String secretString = SecretKeyGenerator.generate();
